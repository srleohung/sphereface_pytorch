...
def train(epoch, args):
    ...
    total = 0
    batch_idx = 0
    print(f'prepare dataset for {epoch+1} training: time={dt()}')
    ...
    print(f'start {epoch+1} training: time={dt()}')
    while True:
        img, label = ds.get()
        if img is None:
            break
        inputs = torch.from_numpy(img).float().to(device)
        targets = torch.from_numpy(label[:, 0]).long().to(device)
        ...
        batch_idx += 1
    print(f'finish {epoch+1} training: time={dt()}')
